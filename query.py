...
q6 = Brand.query.filter(Brand.discontinued == None, Brand.founded==1903).all()
...
def get_model_info(year):
    ...
    if models: 
        for model in models:
            result_str = "Brand is {brand}, Model is {model}, HQ is {HQ}"
            print(result_str.format(brand = model.brand.name, 
                                    model = model.name, 
                                    HQ = model.brand.headquarters))
    else:
        print("There are no models for that year.")
...
def get_brands_summary():
    ...
    for brand in all_brands:
        print(f'Brand name is: {brand.name}')
        if brand.models:
            for model in brand.models:
                print(f'Model is: {model.name}, year is: {model.year}')
